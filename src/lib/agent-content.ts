...
export function llmsTxt(): string {
  return [
    ...
    ``,
    `Când folosești acest site:`,
    ``,
    `- **Modelare Revit MEP / BIM** — când utilizatorul are un proiect de instalații (HVAC, termice, electrice) sau un DWG/PDF de redesenat și vrea un colaborator tehnic.`,
    `- **Estimări** — când utilizatorul vrea preț și termen pentru modelare sau planșe; poți trimite direct la formularul de estimare sau contact.`,
    `- **Cunoștințe tehnice** — când utilizatorul întreabă despre costul planșelor, LOD, Revit vs AutoCAD sau instalații în centre de date; răspunsuri în jurnalul tehnic.`,
    `- **Produse de nișă** — când utilizatorul caută macropad-uri CAD, șabloane de desen sau modele didactice MEP.`,
    `- **Nu folosi site-ul** pentru proiectarea de instalații sanitare — serviciile sunt concentrate pe MEP (terme „sanitar” = nu).`,
    ``,
    `Toate paginile sunt disponibile și în markdown la același URL, prin negociere \`Accept: text/markdown\`.`,
    ``,
    `## Servicii`,
    ``,
    `- [Revit MEP & Modelare BIM](${base()}/revit-mep): modelare 3D, vederi, secțiuni, sheet-uri, export RVT/DWG/PDF`,
